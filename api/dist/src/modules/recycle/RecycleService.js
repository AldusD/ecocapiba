...
export class RecycleService {
    ...
    async registerRecycle(userId, currentDate) {
        try {
            return await this.recycleRepository.create(userId, currentDate);
        }
        catch (error) {
    ...
    async getStreakMultiplier(userId) {
        ...
        const recycledThisWeek = await this.recycleRepository.hasRecycleInPeriod(userId, currentWeekRange.start, currentWeekRange.end);
        if (recycledThisWeek) {
            streakWeeks++;
        }
        ...
        while (keepChecking && streakWeeks < 10) {
            ...
            const hasRecycled = await this.recycleRepository.hasRecycleInPeriod(userId, start, end);
            if (hasRecycled) {
                streakWeeks++;
                checkDate.setDate(checkDate.getDate() - 7);
            }
